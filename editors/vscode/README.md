...
`surinortAst.lspCommand` when the executable is not on `PATH`. The command
palette also provides flowbit navigation, a match-space preview, and optional
engine validation when `surinortAst.engineCommand` contains `{file}`.
